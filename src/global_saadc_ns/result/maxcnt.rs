...
#[doc = "Field `MAXCNT` reader - Maximum number of buffer bytes to transfer. Note that one sample is two bytes."]
pub type MaxcntR = crate::FieldReader<u16>;
#[doc = "Field `MAXCNT` writer - Maximum number of buffer bytes to transfer. Note that one sample is two bytes."]
pub type MaxcntW<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
impl R {
    #[doc = "Bits 0:14 - Maximum number of buffer bytes to transfer. Note that one sample is two bytes."]
    #[inline(always)]
